...
class Model {
    ...
    /**
     * Sets .name property for this class to know the name of the model.
    ...
    /**
     * Sets .XHRModifier property. This modifier may be used to add custom headers to the request.
     * @todo should we make an inheritable method instead of this?
     * @param modifier The modifier. A function that accepts the only parameter xhr, which is a XMLHttpRequest object
     * of current request.
     * @author Mikhail Kormanowsky
     * @since 2.0.0
     */
    static useXHRModifier(modifier) {
        this.XHRModifier = modifier;
    }
    ...
    static apiClient() {
        if (!this._apiClient) {
            ...
            this._apiClient.res(this.name).res(Object.getOwnPropertyNames(this.prototype));
            this._apiClient.on("request", this.XHRModifier);
        }
